...
from .schema import Table, Column, Constraint
from .action import Action

def get_schema_diff_actions(model_schema: List[Table], database_schema: List[Table]):
    actions = []

    # Then look for tables that exist in the model_schema that are not in the database_schema and thus
    # need to be created
    for model_table in model_schema:
        if model_table.name not in [table.name for table in database_schema]:
            actions.append(Action(type="CREATE_TABLE", table=model_table))
            # Find indexes that don't exist in the database
            for index in model_table.indexes:
                actions.append(Action(type="CREATE_INDEX", table=model_table, index=index))
    
    # Look to see if any tables exist in the database_schema that do not exist in the model_schema
    # And thus need to be dropped
    for db_table in database_schema:
        if db_table.name not in [table.name for table in model_schema]:
            actions.append(Action(type="DROP_TABLE", table=db_table))


    for table in model_schema:
        database_table = next(
            (t for t in database_schema if t.name == table.name), None
        )
        ...
        for index in table.indexes:
            ...
            if database_index == None:
                actions.append(Action(type="CREATE_INDEX", table=table, index=index))

        for db_index in database_table.indexes:
            model_index = next((i for i in table.indexes if i == db_index), None)
            if model_index == None:
                actions.append(Action(type="DROP_INDEX", table=database_table, index=db_index))
        ...
        for db_column in database_table.columns:
            if db_column.name not in [column.name for column in table.columns]:
                actions.append(
                    Action(type="DROP_COLUMN", table=db_table, column=db_column)
                )
        ...
        for model_column in table.columns:
            if model_column.name not in [
                ...
                actions.append(
                    Action(type="CREATE_COLUMN", table=table, column=model_column)
                )
        ...
        for column in table.columns:
            ...
            if column.data_type != database_column.data_type:
                actions.append(
                    Action(type="CHANGE_DATA_TYPE", table=table, column=column)
                )

            if (column.default or "").lower() != (database_column.default or "").lower():
                actions.append(
                    Action(type="COLUMN_DEFAULT", table=table, column=column)
                )

            if column.nullable == True and database_column.nullable == False:
                actions.append(
                    Action(type="COLUMN_NULLABLE", table=table, column=column)
                )

            if column.nullable == False and database_column.nullable == True:
                actions.append(
                    Action(type="COLUMN_NOT_NULLABLE", table=table, column=column)
                )
            ...
            for constraint in column.constraints:
                ...
                if database_constraint == None:
                    actions.append(Action(type="ADD_CONSTRAINT", table=table, column=column, constraint=constraint))
            ...
            for constraint in database_column.constraints:
                ...
                if model_constraint == None:
                    actions.append(Action(type="DROP_CONSTRAINT", table=table, column=column, constraint=constraint))
            ...
            for constraint in column.constraints:
                ...
                if constraint != database_constraint:
                    actions.append(Action(type="DROP_CONSTRAINT", table=table, column=column, constraint=database_constraint))
                    actions.append(Action(type="ADD_CONSTRAINT", table=table, column=column, constraint=constraint))
